...
export default function RemoveBgPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/clipdrop/remove-bg", { method: "POST", body: fd })
  ...
  return (
    <main className="min-h-screen bg-[#F5F5F4] text-[#2F3E46]">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-10">
        <header className="mb-6"><h1 className="text-3xl font-semibold">Remove Background</h1><p className="mt-2 text-[#2F3E46]/80">Instant cutouts with clean edges for products and portraits.</p></header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              ...
              <div className="mt-4 flex items-center gap-3">
                <button onClick={run} disabled={!file || loading} className="rounded-xl bg-[#4ECDC4] px-4 py-2 font-semibold text-[#2F3E46] disabled:opacity-50">{loading ? "Processing…" : "Remove background"}</button>
            ...
            <div>
              ...
              {result && <a href={result} download="cutout.png" className="mt-4 inline-block rounded-xl border border-[#2F3E46] px-4 py-2 font-semibold hover:bg-[#EDEDED]">Download</a>}
            </div>
          </div>
        </section>
      </div>
